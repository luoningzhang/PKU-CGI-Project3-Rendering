...
static LRESULT CALLBACK ProcessMessage(HWND hWnd, UINT uMsg, WPARAM wParam,
  ...
  if (renderer == NULL) {
    ...
    if (renderer->button_pressed_[size_t(BUTTON::Left)]) {
      if (renderer->framebuffer_) {
        // renderer->framebuffer_->Clear();
        for (int y = 0; y < renderer->height_; y++) {
          renderer->buffer[y] = new Color[renderer->width_];
          for (int x = 0; x < renderer->width_; x++) {
            renderer->buffer[y][x] = Color::Zero();
          }
        }
        // renderer->window_->DrawBuffer(renderer->framebuffer_);
      }
      ////////////////////////////////////////////////////////////////////////////////////
    ...
    renderer->camera_->Scale(offset/10.0f);
  }
  else {
    return DefWindowProc(hWnd, uMsg, wParam, lParam);
  }
